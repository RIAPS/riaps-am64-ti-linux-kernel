    ...
    stage('Run ARM64 Container') {
      steps {
        script {
          ...
          sh "sudo docker exec ${arm64Container} bash -c 'cd /tmp/riaps-am64-ti-linux-kernel && ./run.sh ti-linux-kernel-rt'"
          ...
          sh "sudo docker cp ${arm64Container}:/tmp/riaps-am64-ti-linux-kernel/build/${debianSuite}/ti-linux-kernel-rt/ti-linux-kernel-rt*/arch/arm64/boot/dts/ti/k3-am642-sk.* ${HOST_OUTPUT_DIR}"
        }
      }
    }
  }
